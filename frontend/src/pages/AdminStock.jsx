        ...
        <div className="mt-4 space-y-4">
          ...
          <form onSubmit={submitNuevoSede} className="grid gap-3 sm:grid-cols-3">
            <div className="sm:col-span-2">
              ...
              />
              <p className="mt-1 text-xs text-subtle">
                Se asigna código SED00x y se crea almacén aduana con gabinete, estante E01 y contenedor C01.
              </p>
            </div>
          ...
          <form onSubmit={submitMapAlmacenSede} className="grid gap-3 border-t border-border pt-4 sm:grid-cols-3">
            <div>
              <label className="text-label">Almacén</label>
              <select className="input-field" value={mapAlmacen} onChange={(e) => setMapAlmacen(e.target.value)}>
                {almacenes.map((a) => (
                  <option key={a.codigo} value={a.codigo}>
                    {a.codigo} — {a.nombre}
                  </option>
